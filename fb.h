...
bool fb_stepper_init(fb_stepper_t *s, fb_t *fb, int32_t x, int32_t y, uint32_t witdh, uint32_t height);
...
bool inline fb_stepper_step_x(fb_stepper_t *s, uint32_t rgba)
{
	...
	if (x > 0 && x < s->max_x
	    && y > 0 && y < s->max_y) {
		p = (x * s->m[0][0] + y * s->m[0][1] + s->m[0][2]) + (x * s->m[1][0] + y * s->m[1][1] + s->m[1][2]) * s->pitch_div_4;
