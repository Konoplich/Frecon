...
fb_stepper_init(fb_stepper_t *s, fb_t *fb, int32_t x, int32_t y, uint32_t width, uint32_t height)
{
	...
	s->x = 0;
	s->y = 0;
	s->pitch4 = s->fb->buffer_properties.pitch >> 2;
	...
	switch (s->fb->buffer_properties.rotation) {
		case DRM_MODE_ROTATE_90:
		case DRM_MODE_ROTATE_270:
			s->mx = s->fb->buffer_properties.height;
			s->my = s->fb->buffer_properties.width;
			break;
		case DRM_MODE_ROTATE_180:
		case DRM_MODE_ROTATE_0:
		default:
			s->mx = s->fb->buffer_properties.width;
			s->my = s->fb->buffer_properties.height;
	}
