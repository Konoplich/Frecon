...
class GlyphSet:
    ...
    def to_c_src(self, out_file):
        ...
        print('#define GLYPH_BITS_PER_PIXEL {}'.format(self.bits_per_pixel), file=out_file)
...
class BdfState:
    ...
    def handle_FONTBOUNDINGBOX(self, match):
        self.out_glyph_set = GlyphSet(int(match.group(1)),
                                      int(match.group(2)), 1) # 1 bit per pixel
