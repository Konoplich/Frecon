...
static unsigned kInternalConnectors[] = {
...
static drmModeConnector* find_first_connected_connector(drm_t* drm)
{
...
static drmModeConnector* find_used_connector_by_type(drm_t* drm, unsigned type)
{
	for (int i = 0; i < drm->resources->count_connectors; i++) {
		drmModeConnector* connector;
...
static drmModeConnector* find_main_monitor(drm_t* drm, uint32_t* mode_index)
{
	...
	do {
		main_monitor_connector = find_used_connector_by_type(drm, kInternalConnectors[i]);
		i++;
	} while (!main_monitor_connector && i < ARRAY_SIZE(kInternalConnectors));
	...
	if (!main_monitor_connector)
		main_monitor_connector =
				find_first_connected_connector(drm);
...
static int drm_score(drm_t* drm)
{
	...
	for (c = 0; c < ARRAY_SIZE(kInternalConnectors); c++)
		if (drm->main_monitor_connector)
			if (drm->main_monitor_connector->connector_type == kInternalConnectors[c]) {
				score++;
				break;
			}
