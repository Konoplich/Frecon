...
static drmModeConnector* find_main_monitor(drm_t* drm, uint32_t* mode_index)
{
	...
	do {
		main_monitor_connector = find_used_connector_by_type(drm, kConnectorPriority[i]);
		i++;
...
/*
 * returns true if connector/crtc/driver have changed and video object have to be re-create
 */
