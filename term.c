...
void term_background(void)
{
	...
	while (!dbus_take_display_ownership() && retry--) {
		LOG(ERROR, "Chrome failed to take display ownership. %s", retry ? "Trying again." : "Giving up, Chrome is probably dead.");
		usleep(500 * 1000);
	}
}
...
void term_foreground(void)
{
	...
	while (!dbus_release_display_ownership() && retry--) {
		LOG(ERROR, "Chrome did not release master. %s", retry ? "Trying again." : "Frecon will steal master.");
		usleep(500 * 1000);
	}
