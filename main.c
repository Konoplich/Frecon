...
static struct option command_options[] = {
	...
	{ "max-vts", required_argument, NULL, FLAG_MAX_VTS },
...
int main_process_events(uint32_t usec)
{
	...
	if (term_exception(terminal, &exception_set)) {
		LOG(INFO, "DDD Exception on term %p VT%u", terminal, term_get_current());
		return -1;
	}
	...
	if (term_is_valid(terminal)) {
		if (term_is_child_done(terminal)) {
			if (terminal == term_get_terminal(TERM_SPLASH_TERMINAL) && !command_flags.enable_vt1) {
				LOG(INFO, "DDD Splash child is done, not restarting");
			...
			LOG(INFO, "DDD Child is done on VT%u, killing terminal %p and creating new one %p", term_get_current(), terminal, new_terminal);
...
int main(int argc, char* argv[])
{
	...
	for (;;) {
		...
		switch (c) {
			...
			case FLAG_MAX_VTS:
				term_set_max_terminals(strtoul(optarg, NULL, 0));
				break;
	...
	if (command_flags.pre_create_vts && command_flags.enable_vts) {
		for (unsigned vt = command_flags.enable_vt1?TERM_SPLASH_TERMINAL:1; vt < term_max_terminals; vt++) {
