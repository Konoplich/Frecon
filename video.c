						...
						drmModeConnector *connector)
{
	...
	if (crtc) {
		result = connector->connection == DRM_MODE_CONNECTED;
		drmModeFreeCrtc(crtc);
	}
		...
		if (prop) {
			if (strcmp(prop->name, "EDID") == 0) {
				...
				if (blob_ptr) {
					...
					memcpy(&new_video->edid, blob_ptr->data, 128);
